{
  ...
  const solutionFP = (N: number, stages: number[]): number[] => {
    ...
    const result = Array.from({ length: N }, (_, i) => {
      const fail = stagedPlayer === 0 ? 0 : counts[i + 1] / stagedPlayer;
      
      stagedPlayer = stagedPlayer - counts[i + 1];
      
      return {stage: i + 1, fail}
    })

    return result.toSorted((a, b) => b.fail - a.fail).map((v) => v.stage);
  };
